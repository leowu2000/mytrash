	  ...
	  <td>
		<table width="100%">
		  <tr align="middle" height="80"> 
		    <td  height="80">		
			  <%if("JPG".equals(mapYxzt_m.get("WJGS").toString().trim().toUpperCase())||"JPG".equals(mapYxzt_m.get("WJGS").toString().trim().toLowerCase())){%>
        	  <img src="media.do?action=image&tablename=TB_PJR_M&media_id=<%=mapYxzt_m.get("ZLBM")%>" onMouseOver="display('<%=altStr1%>','<%=altStr2%>','<%=altStr3%>','<%=altStr4%>','<%=altStr5%>','<%=altStr6%>','<%=altStr7%>',event.x,event.y)" onMouseOut="hide()" style=" border=0; height=<%=mediaheight%>;width=<%=mediawidth%>;; z-index:8;"> 
        	  <%}else{%>

...
<%
	for(int i=0;i<listFxxd_m.size();i++){
	    ...
	    String ext = mapFxxd_m.get("WJGS")==null?"":mapFxxd_m.get("WJGS").toString().trim().toUpperCase();
%>	    
	<tr width="100%">
	  <td>
		<table width="100%">
		...
		<%if("JPG".equals(ext)||"JPEG".equals(ext)){ %>
        	  <img src="media.do?action=image&tablename=TB_FPACTI_M&media_id=<%=mapFxxd_m.get("ZLBM")%>" onMouseOver="display('<%=altStr1%>','<%=altStr2%>','<%=altStr3%>','<%=altStr4%>','<%=altStr5%>','<%=altStr6%>','<%=altStr7%>',event.x,event.y)" onMouseOut="hide()" onClick="window.location='../modules/view/view_fxxd.jsp?media_id=<%=mapFxxd_m.get("ZLBM")%>&title=<%=mapFxxd_m.get("TITLE") %>&dtcdt=<%=mapFxxd_m.get("DTCDT")%>&tbdw=<%=tbdw %>&fileName=<%=fileName %>&WJGS=<%=mapFxxd_m.get("WJGS") %>&DETAIL=<%=mapFxxd_m.get("NRMS") %>'" style="cursor:hand;border=0; height=<%=mediaheight %>;z-index:8;"> 
        <%}else{  %>
        	  <img src="../../images/lx.gif" onMouseOver="display('<%=altStr1%>','<%=altStr2%>','<%=altStr3%>','<%=altStr4%>','<%=altStr5%>','<%=altStr6%>','<%=altStr7%>',event.x,event.y)" onMouseOut="hide()" style="cursor:hand; border=0; height=60;width=100; z-index:8;" onClick="window.location='media.do?action=video&tablename=TB_FPACTI_M&media_id=<%=mapFxxd_m.get("ZLBM")%>'" WIDTH="80" HEIGHT="53"> 
        <%}%>
			</td>
		  </tr>
		  <tr align="middle"> 

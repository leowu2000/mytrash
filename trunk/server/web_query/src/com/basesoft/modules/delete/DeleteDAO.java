...
public class DeleteDAO extends com.basesoft.core.CommonDAO {
	...
	public void deleteXq(String id){
		...
		String delSql_xqfl = "DELETE FROM (SELECT TBNAME from TB_XQFL where XQFLDM=(SELECT XQFLDM from TB_STDMC where DNCNO='" + id + "')) WHERE DNCNO='" + id + "'";
		
		jdbcTemplate.execute(delSql);
		jdbcTemplate.execute(delSql_m);
		jdbcTemplate.equals(delSql_xqfl);
	}
	...
	public void deleteYxzt(String id){
		...
		String delSql_gcfl = "DELETE FROM (SELECT TBNAME from TB_GCLB where GCFLDM=(SELECT GCFLDM from TB_PJRCN where PJRNO='" + id + "')) where PJRNO='" + id + "'";

...
public class BuinessDao {
	/**
	 * 新增 生成工程 编号规则 工程类别编号＋Mid(工程所在河流的代码,2,6)＋Mid(工程所在地区的行政区划代码,1,2)
	 * 如果已对该工程采集过运行信息，则不允许修改该工程类型 工程表：TB_PJ detail：TB_PJRCN
	 * 
	 * 从工程表中获取最大流水号 ：select max(pjno) as MaxValue from tb_pj 找出工程的类型：select
	 * gcflmc from tb_gclb where gcfldm='" & Mid(RstProject!pjnmcd, 1, 1)
	 * 找出工程所在的地区：select provnm from tb_cnt where cntcd=' 找出工程所在的流域：select ctnm
	 * from tb_lysx1 where ctcd='A" & Mid(RstProject!pjnmcd, 2, 1) & "000000'
	 * 找出工程所在的水系：select ctnm from tb_lysx1 where ctcd='A" &
	 * Mid(RstProject!pjnmcd, 2, 2) & "00000' 找出工程所在的一级支流：select ctnm from
	 * tb_lysx where ctcd like 'A" & Mid(RstProject!pjnmcd, 2, 4) & "00%'
	 * 找出工程所在的二级支流：select ctnm from tb_lysx where ctcd like 'A" &
	 * Mid(RstProject!pjnmcd, 2, 6) & "%'
	 */
	/**

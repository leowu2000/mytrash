...
function toEdit(){
	...
	if(result==""){
		...
		if(result.lastIndexOf(",")>0){
			...
			document.forms[0].action="/report/hqxxEdit.jsp?RPJINCD="+result;
			document.forms[0].submit();
		}
	}
...
<% 
	...
	List<HQBean> records = BuinessDao.getAllHqList(path,iswhere); 
...
<input type="hidden" value="TB_QT" name="towhere"/>
<input type="hidden" value="TB_QT" name="searchType"/>
...
<table border="0" align="center" width="95%" cellspacing="1" bgcolor="#CCCCCC">
	...
	<tr  bgcolor="#FFFFFF">
		...
		<td><a href="/report/hqxxView.jsp?RPJINCD=<%=bean.getRPJINCD() %>" title="点击查看详细信息"><%=bean.getWTTT()%></a></td>

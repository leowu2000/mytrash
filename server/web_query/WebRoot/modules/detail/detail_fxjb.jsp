...
<%
	for(int i=0;i<listFxjb_m.size();i++){
	    ...
	    String ext = mapFxjb_m.get("WJGS")==null?"":mapFxjb_m.get("WJGS").toString().trim().toUpperCase();
%>
    <tr width="100%">
	  <td>
	    <table width="100%">
		  ...
		  <tr align="middle" height="80"> 
	        ...
	        <%}else if("MPG".equals(ext)||"AVI".equals(ext)){%>
	       	  <img src="../../images/lx.gif" onMouseOver="display('<%=altStr1%>','<%=altStr2%>','<%=altStr3%>','<%=altStr4%>','<%=altStr5%>','<%=altStr6%>','<%=altStr7%>',event.x,event.y)" onMouseOut="hide()" style="cursor:hand; border=0; height=60;width=100; z-index:8;" onClick="window.location='media.do?action=video&tablename=TB_FXJB_M&media_id=<%=mapFxjb_m.get("ZLBM")%>'" WIDTH="80" HEIGHT="53"> 
	    	<%}else {%>
